...
import src.main.resources.BarHandle;
import src.main.resources.BarHandleImpl;
...
public class EWrapperImpl implements EWrapper {
	...
	public void historicalDataEnd(int reqId, String startDateStr, String endDateStr) {
		...
		List<String> symbols=barHandle.readStringList(globalControl.getTemPath());
		int n=reqId-100;
		String symbol=symbols.get(n);
		String priceBar=symbols.get(symbols.size()-1);
		String downloadPath="E:\\Data\\tempData\\tempHistoricalData"+symbol+".csv";//"E:\\Data\\operateData\\"+symbol+barSize+".csv";//
		String dataSavingPath="E:\\Data\\DownloadData\\"+symbol+"\\"+symbol+priceBar+".csv";
		String operateDataPath="E:\\Data\\operateData\\"+symbol+priceBar+".csv";
		...
		System.out.println("Size of barList: "+barList.size());
		barHandle.saveBarList(barList,downloadPath);
		pivotHandle.addPriceRecords(downloadPath,operateDataPath);
		pivotHandle.addPriceRecords(downloadPath,dataSavingPath);
		GlobalController globalController=new GlobalController("E:\\Data\\operateData\\",eigenScratchAddress,basicScratchAddress,symbol,priceBar);
		globalController.createDailyEigenScratch();
		if(n==symbols.size()-3){
			InAndOutHandle inAndOutHandle=new InAndOutHandleImpl();
			List<Scratch> allEigenscratchList=new ArrayList<>();
			for(int t=0;t<=n;t++){
				String filePath=eigenScratchAddress+symbols.get(t)+priceBar+".csv";
				Scratch dummyScratch=new Scratch(symbols.get(t),0,0,0,0,0);
				allEigenscratchList.add(dummyScratch);
				allEigenscratchList.addAll(inAndOutHandle.readScratchFromCSV(filePath));
			}
			inAndOutHandle.saveScratchListToCSV(allEigenscratchList,"E:\\Data\\allEigenScratches"+priceBar+".csv");
		}
		barList.clear();
	}
	//! [historicaldataend]
